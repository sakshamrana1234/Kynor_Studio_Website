...
export default async function handler(request,response){
  ...
  const required=['name','email','company','service','spend','platform','challenge','consent'];
  ...
  const fields=[['Name',body.name],['Email',email],['Company',body.company],['Website',body.website],['Preferred engagement',body.service],['Monthly paid-social spend',body.spend],['Primary platform',body.platform],['Current creative problem',body.challenge],['References',body.references]];
  const html=`<div style="font-family:Arial,sans-serif;max-width:720px;margin:auto;color:#111214"><p style="font-size:12px;letter-spacing:.14em;text-transform:uppercase;color:#275dff">30-day sprint application / Kynor Studio</p><h1 style="font-size:32px">${escapeHtml(body.company)}</h1>${fields.map(([label,value])=>`<div style="padding:14px 0;border-top:1px solid #d9d9d9"><strong>${label}</strong><p style="white-space:pre-wrap;line-height:1.5">${escapeHtml(value)||'—'}</p></div>`).join('')}</div>`;
